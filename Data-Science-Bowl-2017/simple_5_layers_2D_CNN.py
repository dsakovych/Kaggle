...
conf['level_3_filters'] = 16
conf['level_4_filters'] = 32
conf['level_5_filters'] = 64
...
def load_and_normalize_dicom(path, x, y):
    ...
    dicom_img = dicom_.pixel_array.astype(np.float64)
    mn = dicom_img.min()
    mx = dicom_img.max()
    if (mx - mn) != 0:
...
def build_CNN():
    ...
    model.add(ZeroPadding2D((1, 1), dim_ordering='th'))
    model.add(Convolution2D(conf['level_3_filters'], 3, 3, activation='relu', dim_ordering='th'))
    model.add(ZeroPadding2D((1, 1), dim_ordering='th'))
    model.add(Convolution2D(conf['level_3_filters'], 3, 3, activation='relu', dim_ordering='th'))
    model.add(MaxPooling2D((2, 2), strides=(2, 2), dim_ordering='th'))

    model.add(ZeroPadding2D((1, 1), dim_ordering='th'))
    model.add(Convolution2D(conf['level_4_filters'], 3, 3, activation='relu', dim_ordering='th'))
    model.add(ZeroPadding2D((1, 1), dim_ordering='th'))
    model.add(Convolution2D(conf['level_4_filters'], 3, 3, activation='relu', dim_ordering='th'))
    model.add(MaxPooling2D((2, 2), strides=(2, 2), dim_ordering='th'))

    model.add(ZeroPadding2D((1, 1), dim_ordering='th'))
    model.add(Convolution2D(conf['level_5_filters'], 3, 3, activation='relu', dim_ordering='th'))
    model.add(ZeroPadding2D((1, 1), dim_ordering='th'))
    model.add(Convolution2D(conf['level_5_filters'], 3, 3, activation='relu', dim_ordering='th'))
    model.add(MaxPooling2D((2, 2), strides=(2, 2), dim_ordering='th'))
...
def create_single_model():
    ...
    train_patients, test_patients = get_train_single_fold(train_csv_table, conf['train_test_fraction'])
    ...
    for p in test_patients:
        test_files += glob.glob("data/{}/{}/*.dcm".format(get_dir, p))
    print('Number of valid files: {}'.format(len(test_files)))
    ...
    fit = model.fit_generator(generator=batch_generator_train(train_files, train_csv_table, conf['batch_size']),
                              ...
                              validation_data=batch_generator_train(test_files, train_csv_table, conf['batch_size']),
...
def create_submission(model):
    ...
    sample_subm.to_csv("subm512.csv", index=False)


if __name__ == '__main__':
    model = create_single_model()
    if conf['save_weights'] == 1:
        model.save_weights('mdl512.h5')
